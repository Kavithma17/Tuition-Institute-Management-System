...
const AdminDashboard = () => {
  ...
  const fetchTeachers = async () => {
    try { const res = await axios.get("http://localhost:8088/teachers"); setTeachers(res.data); } 
  ...
  const fetchRecordings = async (courseId) => {
    ...
    try { const res = await axios.get(`http://localhost:8088/recordings/course/${courseId}`); setRecordings(res.data); }
  ...
  const fetchLiveclasses = async (courseId) => {
    ...
    try { const res = await axios.get(`http://localhost:8088/liveclasses/course/${courseId}`); setLiveclasses(res.data); }
    catch (err) { console.error("Error fetching live classes:", err); }
  };

  const fetchTutes = async (courseId) => {
    if (!courseId) return setTutes([]);
    try { const res = await axios.get(`http://localhost:8088/tutes/course/${courseId}`); setTutes(res.data); }
  ...
  const handleTeacherSubmit = async (e) => {
    e.preventDefault();
    try {
      if (editingTeacherId) await axios.put(`http://localhost:8088/teachers/${editingTeacherId}`, teacherForm);
      else await axios.post("http://localhost:8088/teachers", teacherForm);
  ...
  const handleCourseSubmit = async (e) => {
    e.preventDefault();
    try {
      if (editingCourseId) await axios.put(`http://localhost:8088/courses/${editingCourseId}`, courseForm);
      else await axios.post("http://localhost:8088/courses", courseForm);
...
const handleRecordingSubmit = async (e) => {
  ...
  try {
    if (editingRecordingId) {
      ...
      const payload = {
        ...
        videoUrl: recordingForm.url,   // backend calls this videoUrl in entity
        description: recordingForm.description || ""
      };
      await axios.put(`http://localhost:8088/recordings/${editingRecordingId}`, payload);
    } else {
      // ✅ Create with query params
      await axios.post(
        `http://localhost:8088/recordings?courseId=${selectedCourseId}&title=${encodeURIComponent(recordingForm.title)}&url=${encodeURIComponent(recordingForm.url)}&description=${encodeURIComponent(recordingForm.description || "")}`
      );
    }
  ...
  const handleLiveclassSubmit = async (e) => {
    ...
    if (!selectedCourseId) return alert("Select a course!");
    try {
      if (editingLiveclassId) await axios.put(`http://localhost:8088/liveclasses/${editingLiveclassId}`, { ...liveclassForm });
      else await axios.post(`http://localhost:8088/liveclasses?courseId=${selectedCourseId}&title=${encodeURIComponent(liveclassForm.title)}&meetingUrl=${encodeURIComponent(liveclassForm.meetingUrl)}&dateTime=${encodeURIComponent(liveclassForm.dateTime)}`);
  ...
  const handleTuteSubmit = async (e) => {
    ...
    if (!selectedCourseId) return alert("Select a course!");
    try {
      if (editingTuteId) await axios.put(`http://localhost:8088/tutes/${editingTuteId}`, { ...tuteForm });
      else await axios.post(`http://localhost:8088/tutes?courseId=${selectedCourseId}&title=${encodeURIComponent(tuteForm.title)}&fileUrl=${encodeURIComponent(tuteForm.fileUrl)}`);
  ...
  return (
    ...
    <button onClick={() => document.getElementById("teachers-section").scrollIntoView({ behavior: "smooth" })}>Teachers</button>
    <button onClick={() => document.getElementById("courses-section").scrollIntoView({ behavior: "smooth" })}>Courses</button>
    <button onClick={() => document.getElementById("recordings-section").scrollIntoView({ behavior: "smooth" })}>Recordings</button>
    <button onClick={() => document.getElementById("liveclasses-section").scrollIntoView({ behavior: "smooth" })}>Live Classes</button>
    <button onClick={() => document.getElementById("tutes-section").scrollIntoView({ behavior: "smooth" })}>Tutes</button>
  </div>
        ...
        <table className="course-table">
          ...
          <tbody>
            {teachers.map(t => <tr key={t.id}><td>{t.name}</td><td>{t.qualification}</td><td>{t.subject}</td><td>{t.photoUrl ? <img src={t.photoUrl} width="50" /> : "No Photo"}</td><td>
              <button onClick={() => { setEditingTeacherId(t.id); setTeacherForm(t); }}>Edit</button>
              <button onClick={() => handleDelete("http://localhost:8088/teachers", t.id, fetchTeachers)}>Delete</button>
            </td></tr>)}
          </tbody>
        </table>
      </section>
        ...
        <table className="course-table">
          ...
          <tbody>
            {courses.map(c => <tr key={c.id}><td>{c.classname}</td><td>{c.month}</td><td>{c.price}</td><td>{c.teachername}</td><td>{c.photourl ? <img src={c.photourl} width="50" /> : "No Image"}</td><td>
              <button onClick={() => { setEditingCourseId(c.id); setCourseForm(c); }}>Edit</button>
              <button onClick={() => handleDelete("http://localhost:8088/courses", c.id, fetchCourses)}>Delete</button>
            </td></tr>)}
          </tbody>
        </table>
      </section>
      ...
      <section id="recordings-section">
        ...
        <table className="course-table">
          ...
          <tbody>
            {recordings.map(r => <tr key={r.id}><td>{r.title}</td><td>{r.videoUrl} </td><td>{r.description}</td><td>
              <button onClick={() => { setEditingRecordingId(r.id); setRecordingForm({ title: r.title, url: r.videoUrl, description: r.description }); }}>Edit</button>
              <button onClick={() => handleDelete("http://localhost:8088/recordings", r.id, fetchRecordings)}>Delete</button>
            </td></tr>)}
          </tbody>
        </table>
      </section>
      ...
      <section id="liveclasses-section">
        ...
        <table className="course-table">
          ...
          <tbody>
            {liveclasses.map(l => <tr key={l.id}><td>{l.title}</td><td>{l.meetingUrl}</td><td>{l.dateTime}</td><td>
              <button onClick={() => { setEditingLiveclassId(l.id); setLiveclassForm({ title: l.title, meetingUrl: l.meetingUrl, dateTime: l.dateTime }); }}>Edit</button>
              <button onClick={() => handleDelete("http://localhost:8088/liveclasses", l.id, fetchLiveclasses)}>Delete</button>
            </td></tr>)}
          </tbody>
        </table>
      </section>
      ...
      <section id="tutes-section">
        ...
        <table className="course-table">
          ...
          <tbody>
            {tutes.map(t => <tr key={t.id}><td>{t.title}</td><td>{t.fileUrl} </td><td>
              <button onClick={() => { setEditingTuteId(t.id); setTuteForm({ title: t.title, fileUrl: t.fileUrl }); }}>Edit</button>
              <button onClick={() => handleDelete("http://localhost:8088/tutes", t.id, fetchTutes)}>Delete</button>
            </td></tr>)}
          </tbody>
        </table>
      </section>
